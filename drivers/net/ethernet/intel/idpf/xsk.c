...
/**
 * idpf_xsksq_clean - clean an XSk Tx queue
 * @xdpq: XSk Tx queue
 */
...
/**
 * idpf_xsksq_complete - produce AF_XDP descriptors to CQ
 * @_xdpq: XDP Tx queue
 * @budget: maximum number of descriptors to clean
 *
 * Return: number of cleaned descriptors.
 */
static __always_inline u32 idpf_xsksq_complete(void *_xdpq, u32 budget)
{
...
static u32 idpf_xsk_tx_prep(void *_xdpq, struct libeth_xdpsq *sq)
{
	...
	if (free < xdpq->thresh)
		free += idpf_xsksq_complete(xdpq, xdpq->thresh);
