...
/**
 * idpf_xdpsq_set_rs - set RS bit on last produced descriptor
 * @xdpq: XDP queue to produce the HW Tx descriptors on
 */
static inline void idpf_xdpsq_set_rs(const struct idpf_tx_queue *xdpq)
{
	u32 ntu, cmd;
...
/**
 * idpf_xdpsq_update_tail - update the XDP Tx queue tail register
 * @xdpq: XDP Tx queue
 */
static inline void idpf_xdpsq_update_tail(const struct idpf_tx_queue *xdpq)
{
	dma_wmb();
...
static inline void idpf_xdp_tx_finalize(void *_xdpq, bool sent, bool flush)
{
	...
	idpf_xdpsq_set_rs(xdpq);
	idpf_xdpsq_update_tail(xdpq);
