...
static void idpf_tx_buf_rel_all(struct idpf_queue *txq)
{
	...
	for (i = 0; i < txq->desc_count; i++)
		libie_tx_complete_any(&txq->tx_buf[i], txq->dev, &bq, &ss);
...
static void idpf_vport_xdpq_get(const struct idpf_vport *vport)
{
...
static void idpf_vport_xdpq_put(const struct idpf_vport *vport)
{
	if (!idpf_xdp_is_prog_ena(vport))
		return;
			     ...
			     struct idpf_vport_max_q *max_q)
{
	 ...
	 * additional Tx queues for XDP.
	 */
	num_xdpq = libie_xdp_get_sq_num(user->num_req_tx_qs, max_q->max_txq);
...
static int idpf_rx_splitq_clean(struct idpf_queue *rxq, int budget)
{
	...
	if (rxq->xdp.data)
		...
		xdp_init_buff(&xdp, rxq->truesize, &rxq->xdp_rxq);
...
bypass_hsplit:
		...
		if (hdr_len) {
			struct libie_rx_buffer *hdr;

			hdr = &rx_bufq->rx_buf.hdr_buf[buf_id];
			libie_xdp_process_buf(&xdp, hdr, hdr_len);
			hdr->page = NULL;
		...
		libie_xdp_process_buf(&xdp, rx_buf, pkt_len);
		rx_buf->page = NULL;
