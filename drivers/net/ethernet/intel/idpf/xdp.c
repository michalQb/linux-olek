...
/**
 * idpf_xdp_copy_prog_to_rqs - set pointers to XDP program for each Rx queue
...
/**
 * idpf_xdpsq_complete - Reclaim a batch of TX resources from completed XDP_TX
 * @_xdpq: XDP Tx queue
 * @budget: maximum number of descriptors to clean
 *
 * Return: number of cleaned descriptors.
 */
static u32 idpf_xdpsq_complete(void *_xdpq, u32 budget)
{
...
static u32 idpf_xdp_tx_prep(void *_xdpq, struct libeth_xdpsq *sq)
{
	...
	if (free < xdpq->thresh)
		free += idpf_xdpsq_complete(xdpq, xdpq->thresh);
