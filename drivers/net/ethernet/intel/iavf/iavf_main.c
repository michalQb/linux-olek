...
EXPORT_SYMBOL(iavf_xdp_locking_key);
...
/**
 * iavf_get_rx_buf_len - Compute Rx buffer length according to adapter settings
 * @adapter: board private structure
 *
...
{
	unsigned int queue_idx = rx_ring->queue_index;
	u16 num_buffs = IAVF_DESC_UNUSED(rx_ring);
	struct iavf_hw *hw = &adapter->hw;
	int err;
	...
	RCU_INIT_POINTER(rx_ring->xdp_prog, adapter->xdp_prog);
	iavf_alloc_rx_buffers(rx_ring, num_buffs);
}
...
static void iavf_configure_rx(struct iavf_adapter *adapter)
{
	...
	rx_buf_len = iavf_get_rx_buf_len(adapter);
 ...
 */
void iavf_set_xdp_queue_vlan_tag_loc(struct iavf_adapter *adapter)
{
	int i;
...
static void iavf_init_config_adapter(struct iavf_adapter *adapter)
{
	...
	adapter->af_xdp_zc_qps = bitmap_zalloc(adapter->num_active_queues + adapter->num_xdp_tx_queues, GFP_KERNEL);
 ...
 */
static void iavf_copy_xdp_prog_to_rings(struct iavf_adapter *adapter)
{
	int i;
				  ...
				  struct bpf_prog *prog)
{
	...
	iavf_map_rings_to_vectors(adapter);
	for (i = 0; i < adapter->num_active_queues; i++)
		if (adapter->rx_rings[i].xsk_pool)
...
static int iavf_setup_xdp(struct iavf_adapter *adapter, struct bpf_prog *prog,
			  struct netlink_ext_ack *extack)
{
	unsigned int frame_size = adapter->netdev->mtu + IAVF_PACKET_HDR_PAD;
	bool needs_reconfig = !!prog != iavf_adapter_xdp_active(adapter);
	bool was_running = netif_running(adapter->netdev);
	unsigned int rx_buf_len = iavf_get_rx_buf_len(adapter);
	int err;

	/* buffer length is same for all rings */
	if (frame_size > rx_buf_len) {
