			     ...
			     u64 qword, struct libie_rx_ptype_parsed parsed)
{
	...
	ipv4 = (parsed.outer_ip == LIBIE_RX_PTYPE_OUTER_IP) &&
	       (parsed.outer_ip_ver == LIBIE_RX_PTYPE_OUTER_IPV4);
	ipv6 = (parsed.outer_ip == LIBIE_RX_PTYPE_OUTER_IP) &&
	       (parsed.outer_ip_ver == LIBIE_RX_PTYPE_OUTER_IPV6);
			 ...
			 struct libie_rx_ptype_parsed parsed)
{
	...
	if ((qword & rss_mask) == rss_mask) {
		...
		libie_set_skb_hash(skb, hash, parsed);
	}
}
 ...
 * @qword: `wb.qword1.status_error_len` from the descriptor
 **/
static bool iavf_is_non_eop(struct iavf_ring *rx_ring, u64 qword)
{
...
no_skb:
		...
		if (!skb || iavf_is_non_eop(rx_ring, qword))
			continue;
